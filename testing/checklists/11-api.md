...
**Postman (скачай с postman.com, бесплатно):**
Создай новую коллекцию, добавь запросы:
- POST /api/files: Body → form-data, поле files → file (выбрать файл), output_format → text, initial_prompt → (опционально)
- GET /api/status/{task_id}: task_id подставляй из ответа предыдущего запроса
- POST /api/cancel/{task_id}: для отмены
- GET /api/gpu: без параметров
- GET /api/ollama: без параметров
- GET /api/open-output: без параметров
- GET /api/download/{task_id}/{filename}: скачать результат

Удобно — сохранил коллекцию, потом просто запускаешь.

**curl (если нет Postman или лень):**
Загрузить файл:
curl -X POST -F "files=@test.mp3" -F "output_format=docx" http://127.0.0.1:5000/api/files
Проверить GPU:
curl http://127.0.0.1:5000/api/gpu

**DevTools (F12):**
Network — сделай запрос через UI, найди его в списке, кликни:
- Headers — статус (200, 400, 404), заголовки
- Response — тело ответа (JSON)

**Postman Runner:**
в Postman открой Runner, выбери коллекцию, поставь итераций 2, запусти — пошлёт 2 запроса почти одновременно. Смотри как сервер обработал.
...
- [ ] multipart/form-data: 0 файлов — 400 / ошибка валидации
...
- [ ] Несуществующий task_id — 404
- [ ] Двойной cancel — второй раз 200? (идемпотентность)
...
- [ ] filename с пробелами/кириллицей — корректный URL encode
