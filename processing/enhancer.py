...
def _llama_call():
    ...
    try:
        ...
        _llama_inflight -= 1
        # Простой считаем с конца генерации, а не с её старта: иначе
        # длинный запрос «съедает» бюджет простоя прямо во время работы,
        # и движок гаснет сразу после завершения улучшения.
        _touch_llama()
...
class LlamaCppEnhancer(BaseEnhancer):
    ...
    def _start_server(
        ...
        # пользователь вручную, чтобы не спавнить второй раз с загрузкой модели
        # в память) — используем его. Порт двигался: 8081 (8080 у Apache), потом
        # 33333 (зона 8080-8130 конфликтная).
