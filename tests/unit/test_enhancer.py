...
def test_enhance_calls_ollama(enhancer, mocker):
    mock_post = mocker.patch("processing.enhancer.requests.post", return_value=make_mock_response())
...
def test_enhance_returns_string(enhancer, mocker):
    mocker.patch(
        "processing.enhancer.requests.post", return_value=make_mock_response(text="хороший текст")
    )
...
def test_enhance_error_raises(enhancer, mocker):
    mocker.patch(
        "processing.enhancer.requests.post", side_effect=requests.ConnectionError("timeout")
    )
...
def test_multi_pass_three_calls(enhancer, mocker):
    mock_post = mocker.patch("processing.enhancer.requests.post")
